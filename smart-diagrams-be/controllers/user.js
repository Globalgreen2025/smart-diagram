...
const linkedInCallback = async (req, res) => {
    try {
        ...
        if (!code) {
            return res.redirect('https://smart-diagram-three.vercel.app/login?error=no_code');
        }
        ...
        if (!userData) {
            return res.redirect('https://smart-diagram-three.vercel.app/login?error=auth_failed');
        }
